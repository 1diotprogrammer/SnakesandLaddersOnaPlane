  ...
  implements GGButtonListener
{
  ...
  void prepareRoll(int currentIndex)
  {
    ...
    else
    {
      ...
      // Task 3, if two puppet is overlap, old one should go back
  ...
  // Task 1, avoid if we have more than one dice, the hand can not be click
  private int numHandClicked = 0;
  public void buttonClicked(GGButton btn)
  {
    System.out.println("hand button clicked");
    numHandClicked ++;
    if (numHandClicked == getNumberOfDice())
      prepareBeforeRoll();
    roll(getDieValue());
  }
  ...
  private void roll(int rollNumber)
  {
    ...
    // Task 1, after the last dice roll, move the puppet
